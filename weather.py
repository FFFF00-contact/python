...
def xu_li1(response1):
	...
	for i in range(0,8):
		...
		day=response1.json()["daily"][i]["temp"]["day"]
		night=response1.json()["daily"][i]["temp"]["night"]
		day1=response1.json()["daily"][i]["feels_like"]["day"]
		night1=response1.json()["daily"][i]["feels_like"]["night"]
		...
		print(Fore.LIGHTGREEN_EX+f"Khu vuc: {khu_vuc}\nBan ngay: {day} °c\nBan dem: {night} °c")
		print(Fore.LIGHTBLUE_EX+f"Ban ngay cam giac nhu: {day1} °c\nBan dem cam giac nhu: {night1} °c")
